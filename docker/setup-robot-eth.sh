#!/bin/bash
# Ethernet to Go2 — use after cable + static IP on host (see setup-host-eth.sh)
set +u
if [[ -f /opt/ros/humble/setup.bash ]]; then
  source /opt/ros/humble/setup.bash
  export PATH="/opt/ros/humble/bin:${PATH}"
else
  echo "ERROR: install-deps.sh nav2"
...
if ! ping -c1 -W1 "${GO2_ROBOT_IP}" &>/dev/null; then
  echo "WARN: no ping to ${GO2_ROBOT_IP} — on HOST run: ./setup-host-eth.sh (cable + robot ON)"
fi
